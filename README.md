...
- Принудительная смена пароля администратора при входе с паролем по умолчанию: остальные API блокируются до смены пароля.
- Защита от потери данных анкеты: локальный черновик, предупреждение при закрытии вкладки и индикатор несохранённых изменений.
- Быстрое продолжение работы: последние кейсы на стартовом экране рабочего места.
- Уведомления о готовности фоновых AI-заданий: тост, бейдж в заголовке вкладки и браузерное уведомление.
- Референсные интервалы у числовых полей с подсветкой значений вне диапазона при вводе.
- Понятные ошибки AI-анализа с советом по устранению и кнопкой повтора.
...
```bash
scripts/build_release.sh --version v0.9.11
```

Скрипт запускает Python-тесты, собирает `dist/cvd-web-v0.9.11.tar.gz` и создаёт рядом `.sha256`. Этот архив можно передать в WSL2 или на VPS, распаковать и запустить bundled `install.sh`.
...
scripts/install_from_release.sh \
  --url https://storage.example.com/cvd-web/v0.9.11/cvd-web-v0.9.11.tar.gz \
...
scripts/publish_github_release.sh \
  --tag v0.9.11 \
  --archive /workspace/cvd-web-release/cvd-web-v0.9.11.tar.gz
```
...
Диагностика AI Gateway проверяет каталог моделей, выбранную модель, загруженное состояние, latency и текущие auth headers из формы без раскрытия их значений пользователям. Явное значение `ai_gateway_headers_json=[]` отключает legacy headers `ai_gateway_auth_header_name/value`. HTTP-вызовы к LM Studio отправляют дефолтный `User-Agent: CVD-Web/<version>`, чтобы Cloudflare не блокировал Python urllib как `Error 1010`. Длинные генерации читаются через OpenAI-compatible streaming/SSE, поэтому cloudflared получает чанки ответа и не ждёт один большой финальный JSON до Cloudflare timeout `524`.
...
Кнопка `Подготовить текст с AI` принимает свободную медицинскую запись и ставит её в персистентную AI-очередь: обработка продолжается после закрытия окна, а активные задания подготовки текста и диагноза видны в рабочем UI. Полный исходный текст временно хранится в SQLite только пока фоновому worker-у нужно выполнить задание; после завершения запись задания оставляет короткий preview, SHA-256, длительность, токены, число сопоставленных полей и результат подготовки для ручной проверки. Сервер отбрасывает неизвестные пути, `MODEL_OUTPUT`, некорректные числа и коды МКБ-10. Результат применяется только после ручного выбора полей.
...
Umbrel package находится в `granyov-cvd-web/` и использует image `ghcr.io/granyov/cvd-web:v0.9.11`. GitHub Actions workflow `Docker Image` публикует этот image в GHCR на tag `v*` или при ручном запуске workflow. Иконка приложения задаётся абсолютным URL `https://raw.githubusercontent.com/granyov/cvd-web/main/granyov-cvd-web/icon.svg`, чтобы Community App Store корректно показывал её в UI Umbrel.
