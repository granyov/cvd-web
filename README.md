...
- FHIR R4 JSON export кейса вместе с заключением: DiagnosticReport с кодами МКБ-10, ClinicalImpression с рассуждением AI, CarePlan-черновик рекомендаций, Condition диагноза врача со статусом confirmed.
- «Копировать для МИС»: готовый текстовый блок протокола (диагноз врача, черновик AI, коды, обоснование, рекомендации) для вставки в ЕМИАС и другие МИС.
...
```bash
scripts/build_release.sh --version v0.9.15
```

Скрипт запускает Python-тесты, собирает `dist/cvd-web-v0.9.15.tar.gz` и создаёт рядом `.sha256`. Этот архив можно передать в WSL2 или на VPS, распаковать и запустить bundled `install.sh`.
...
scripts/install_from_release.sh \
  --url https://storage.example.com/cvd-web/v0.9.15/cvd-web-v0.9.15.tar.gz \
...
scripts/publish_github_release.sh \
  --tag v0.9.15 \
  --archive /workspace/cvd-web-release/cvd-web-v0.9.15.tar.gz
```
...
Загружайте модель в LM Studio с контекстом не меньше **32 768 токенов**: типовой кейс с анамнезом, описаниями ЭКГ, ЭхоКГ и визуализации занимает 3–15 тысяч токенов, а на ответ дополнительно резервируется `lm_studio_max_tokens`. При 8 192 токенах на данные остаётся около 4 000, и объёмные случаи не помещаются.

Приложение узнаёт контекст загруженной модели само: health-check в админке сохраняет его в `lm_studio_context_tokens`, окно «Проверка перед отправкой в AI» показывает оценку объёма случая, а слишком большой случай отклоняется до постановки в очередь — врач не ждёт минуту ради ошибки. Если модель перезагрузили с другим контекстом, значение перечитывается автоматически перед отказом, поэтому устаревшая настройка не блокирует работу.
...
Umbrel package находится в `granyov-cvd-web/` и использует image `ghcr.io/granyov/cvd-web:v0.9.15`. GitHub Actions workflow `Docker Image` публикует этот image в GHCR на tag `v*` или при ручном запуске workflow. Иконка приложения задаётся абсолютным URL `https://raw.githubusercontent.com/granyov/cvd-web/main/granyov-cvd-web/icon.svg`, чтобы Community App Store корректно показывал её в UI Umbrel.
